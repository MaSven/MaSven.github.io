...
function makeOpener (seed) {
   return "Build the digital future of architecture together";
}

function makeSubDescs (seed) {
    var results = [];
    results.push("What makes " + "Softarchitect" + " *? Our commitment to + pursuing excellence at every turn.");
    results.push("You know what you want, something * that doesn't hold you back. With the latest * technologies, " + "Softarchitect" + " has your back.");
    results.push("Unlike our competitors, we'll always be + looking for new ways to move forward.");
    results.push("Softarchitect" + " is * from the ground up. We have * products, * prices, and most of all * service.");
    results.push("Softarchitect" + " is a different kind of startup. We know what it means to be * and will never compromise when it comes to + delivering what you need.");
    ...
    results.push("Softarchitect" + " is inherently * and community centric. It's time for the * community to have a voice.");
    ...
    results.push("Softarchitect" + " is easy to use, no matter who you are.");
    results.push("Softarchitect" + " customers love the product and the company. We love them too!");
    results.push("We provide 24x7x365 support with knowledgeable " + "Softarchitect" + " staff answering your requests.");
    ...
    results.push("Softarchitect" + " is a simple and powerful tool that automatically organizes for * results.");
...
function thanksTo (seed) {
    if (seedChoice(seed, nouns) == commonWord(seed)) {
        return "Thanks to " + "Softarchitect" + " we now have beautiful, " + seedChoice(seed + 4, bizAdjs) + " " + seedChoice(seed, plurals) + " every day. Our customers are happy and engagement has drastically increased. Highly recommend!";
    } else {
        return "I've been " + seedChoice(seed, gerunds) + " for 15 years and I've never seen anything like " + "Softarchitect" + ". They're really something else.";
    }
}

function allMy (seed) {
    if (seedChoice(seed, nouns) == commonWord(seed)) {
        return "When my friends need new " + seedChoice(seed, plurals) + " there's only one word I tell them, " + "Softarchitect" + ".";
    } else {
        return "As a professional in the " + seedChoice(seed, gerunds) + " industry, I tell all my new clients to start out with an account on " + "Softarchitect" + ". It puts them miles ahead of the competition and makes my job that much easier!";
    }
}

function forMe (seed) {
    if (seedChoice(seed, nouns) == commonWord(seed)) {
        return "We’ve tried every " + seedChoice(seed, nouns) + " program available. " + "Softarchitect" + " was the easiest to setup & the most effective.";
    } else {
        return capitalizeFirst(seedChoice(seed, gerunds)) + " apps are not for me. Being recommended by a friend on " + "Softarchitect" + " is exactly what I need.";
    }
}

function toLearn (seed) {
    if (seedChoice(seed, nouns) == commonWord(seed)) {
        return "When I needed a new " + seedChoice(seed, nouns) + " my best friend told me about " + "Softarchitect" + ". I gave it a try and was completely thrilled!";
    } else {
        return "Softarchitect" + " is a great idea, I started " + seedChoice(seed, gerunds) + " to learn this skill set! ";
    }
}

function tinderFor (seed) {
    if (seedChoice(seed, nouns) == commonWord(seed)) {
        return "Softarchitect" + " is like Tinder... but for " + seedChoice(seed, plurals) + "!";
    } else {
        return "Softarchitect" + " is like Tinder... but for " + seedChoice(seed, gerunds) + "!";
    }
}

function facebookFor (seed) {
    if (seedChoice(seed, nouns) == commonWord(seed)) {
        return "Softarchitect" + " is the Facebook of " + seedChoice(seed, plurals) + ".";
    } else {
        return "Softarchitect" + " is the Facebook of " + seedChoice(seed, gerunds) + ".";
    }
}

function makeTest (seed, n) {
    var results = [];
    results.push("Softarchitect" + " was " + seedChoice(seed + 4, bizAdjs) + ", " + seedChoice(seed + 5, bizAdjs) + ", and " + seedChoice(seed + 6, bizAdjs) + ". Ten out of ten!");
    results.push("I was skeptical at first but " + "Softarchitect" + " really came through. They answered all my questions and made the whole experience a plesant one.");
    results.push(thanksTo(seed));
    results.push(allMy(seed));
    results.push("One of the biggest challenges we faced after we hit our early goals was how do we stay top-of-mind and keep our users engaged? Enter " + "Softarchitect" + ". Problem solved.");
    results.push(forMe(seed));
    results.push(toLearn(seed));
    results.push("Softarchitect" + " is the real deal. No doubt about it.");
    ...
    results.push("Softarchitect" + " was very useful not only in case of our project but just good basis for every business we would start in the future.");
    ...
    results.push("Once again, " + "Softarchitect" + " came through. And even before estimated delivery date! Just impressive! Thank you! You’re awesome!");
    results.push("Softarchitect" + " was excellent - especially relative to other services I tried - and certainly 1000x better than the experiences I had with other providers.");
    ...
    results.push("It feels good to be part of the " + "Softarchitect" + " family.");
    results.push("We searched through a lot of providers and plans to find a good fit for our business. " + "Softarchitect" + " made it easy to find the right fit for my business and my budget.");
...
function beTheFirst(seed) {
    ...
    return seedChoice(seed + 14, results) + "Softarchitect";
}
